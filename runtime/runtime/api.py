...
class Actions(StudentAPI):
    ...
    An "action" is a special function defined with the ``async def`` keywords, like so:
        ...
        >>> async def wave_hand():
        ...     Robot.set_value(SERVO_UID, 'servo0', -0.5)
...
class Robot(DeviceAPI):
    ...
    def get_value(self, uid: Union[str, int], param: str) -> Any:
        return self.buffers[int(uid)].get_value(param)

    def set_value(self, uid: Union[str, int], param: str, value: Any):
        self.buffers[int(uid)].set_value(param, value)


class Gamepad(DeviceAPI):
    def get_value(self, param: str, index: int = 0) -> Any:
        return self.buffers['gamepad', index].get_value(param)
