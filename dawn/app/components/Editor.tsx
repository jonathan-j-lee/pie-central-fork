...
const EditorStatus = (props) => (
  <div className="editor-status">
    ...
    >
      <code>{props.filePath ?? '(Unsaved File)'}{props.dirty ? '*' : ''}</code>
    </Tooltip>
    {props.range.chars > 0 && <Tooltip
      className={`cursor-status-tooltip ${Classes.TOOLTIP_INDICATOR}`}
      content={<p className="tooltip-content">
        Selected text spans {props.range.rows} lines and {props.range.chars} characters.
      </p>}>
      <code>({props.range.rows}, {props.range.chars})</code>
    </Tooltip>}
    <Tooltip
      className={`cursor-status-tooltip ${Classes.TOOLTIP_INDICATOR}`}
      content={<p className="tooltip-content">
    ...
    >
      <code>{props.cursor.row + 1}:{props.cursor.column + 1}</code>
    </Tooltip>
  </div>
);
...
export default function Editor(props) {
  ...
  React.useEffect(() => {
    ...
    const timeout = setTimeout(() => dispatch(initializeSettings({ editor: props.editor })), 100);
