...
const logIn = (username = 'admin', password = 'test', status = 200) =>
  agent.post('/login').send({ username, password }).expect(status);

const logOut = () => agent.post('/logout').expect(200);
...
describe('user/session data', () => {
  ...
  it('should set session data', async () => {
    await agent
      ...
      .expect(200);
    let response = await agent.get('/session').expect(200);
...
describe.each([
  ['/alliances', { name: 'Alliance' }, { name: 'Alliance 2' }],
  [
    '/teams',
  ...
  it('performs basic CRUD operations on entities', async () => {
    let response = await agent.get(endpoint).expect(200);
    expect(response.body).toMatchObject([]);
    await agent.put(endpoint).send([data, data]).expect(200);
    response = await agent.get(endpoint).expect(200);
    ...
    response = await agent.delete(endpoint).send([entity1.id]).expect(200);
  ...
  it('rejects a bad payload', async () => {
    let response = await agent.put(endpoint).send(data).expect(400);
    expect(response.body.err).toBeTruthy();
    response = await agent.put(endpoint).expect(400);
    expect(response.body.err).toBeTruthy();
    response = await agent.delete(endpoint).expect(400);
    expect(response.body.err).toBeTruthy();
  });
  ...
  it('rejects unauthenticated writes', async () => {
    const response = await agent.put(endpoint).send([data]).expect(200);
    const [entity] = response.body;
    await logOut();
    await agent.put(endpoint).send([data]).expect(401);
    await agent.delete(endpoint).send([entity.id]).expect(401);
  });
});
...
describe('entity relationships', () => {
  ...
  beforeEach(async () => {
    ...
    await agent
      ...
      .expect(200);
    response = await agent.get('/matches').expect(200);
    match = response.body[0];
  });

  it('casacdes alliance deletion', async () => {
    await agent.delete('/alliances').send([alliance.id]).expect(200);
    const response = await agent.get('/teams').expect(200);
    expect(response.body).toMatchObject([{ name: 'Berkeley', alliance: null }]);
  });

  it('casacdes team deletion', async () => {
    await agent.delete('/teams').send([team.id]).expect(200);
    const response = await agent.get('/matches').expect(200);
    expect(response.body).toMatchObject([{ events: [] }]);
  });

  it('cascades bracket deletion', async () => {
    await agent.post('/bracket').expect(200);
    let response = await agent.get('/bracket').expect(200);
    ...
    await agent.delete('/bracket').expect(200);
    response = await agent.get('/matches').expect(200);
  ...
  it('prevents alliance deletion when it exists in a bracket', async () => {
    await agent.post('/bracket').expect(200);
    await agent.delete('/alliances').send([alliance.id]).expect(200);
    const response = await agent.get('/alliances').expect(200);
...
describe.each([
  ...
  'bracket generation with %s alliances',
  (numAlliances, expected: SymmetricFixture | null) => {
    ...
    function matchesTree(
      ...
      expected: SymmetricFixture | null
    ): boolean {
      if (!current || !expected) {
      ...
      return (
        (matchesTree(current.blue, child1) && matchesTree(current.gold, child2)) ||
    ...
    it('generates the correct structure', async () => {
      ...
      await agent.post('/bracket').send(allianceIds).expect(200);
      response = await agent.get('/bracket').expect(200);
    ...
    it('can delete a bracket', async () => {
      ...
      await agent.delete('/bracket').expect(200);
      await agent.delete('/bracket').expect(200);
      const response = await agent.get('/bracket').expect(200);
      expect(response.body).toBeNull();
    });
    ...
    it('rejects unauthentiated bracket creation/deletion', async () => {
      await agent.post('/bracket').send(allianceIds).expect(200);
      await logOut();
      await agent.post('/bracket').expect(401);
      await agent.delete('/bracket').expect(401);
      await logIn();
      const response = await agent.get('/bracket').expect(200);
...
function searchBracket(
  ...
  if (
    (blue === alliance1 && gold === alliance2) ||
    (blue === alliance2 && gold === alliance1)
  ) {
    return fixture;
  }
  return (
    searchBracket(fixture.blue, alliance1, alliance2) ??
