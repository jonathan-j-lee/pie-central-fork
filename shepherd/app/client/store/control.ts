...
export const connectTeam = createAsyncThunk<
  ...
  { state: RootState }
>('control/connectTeam', async ({ alliance, teamId, hostname }, thunkAPI) => {
  const {
    control: { matchId },
    teams,
    matches,
...
export const disconnectTeam = createAsyncThunk<
  ...
  { teamId: number },
  { state: RootState }
>('control/disconnectTeam', async ({ teamId }, thunkAPI) => {
  const {
    control: { matchId },
    matches,
  } = thunkAPI.getState();
...
export const changeMode = createAsyncThunk<
  ...
  { state: RootState }
>('control/changeMode', async ({ mode, totalTime, robots }, thunkAPI) => {
  const {
    control: { matchId },
  } = thunkAPI.getState();
  if (matchId !== null) {
...
export const adjustScore = createAsyncThunk<
  ...
  { state: RootState }
>('control/adjustScore', async ({ alliance, points }, thunkAPI) => {
  const {
    control: { matchId },
    matches,
  } = thunkAPI.getState();
...
export const extendMatch = createAsyncThunk<
  ...
  { state: RootState }
>('control/extend', async ({ extension, robots }, thunkAPI) => {
  if (extension > 0) {
    const {
      control: { matchId },
    } = thunkAPI.getState();
...
const slice = createSlice({
  ...
  extraReducers(builder) {
    builder
      ...
      .addMatcher(isAnyOf(refresh.fulfilled, refresh.rejected), (state) => ({
        ...state,
        loading: false,
      }));
  },
